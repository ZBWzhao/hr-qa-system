<template>
  <el-card>
    ...
    <el-tabs v-model="activeTab">
      ...
      <el-tab-pane label="制度文档" name="documents">
        <div style="display: flex; gap: 12px; margin-bottom: 16px">
          <el-input v-model="docFilters.keyword" placeholder="搜索标题或内容..." clearable style="width: 260px" @keyup.enter="fetchDocuments" @clear="fetchDocuments">
            <template #append>
              <el-button :icon="Search" @click="fetchDocuments" />
            </template>
          </el-input>
          <el-select v-model="docFilters.category" placeholder="全部分类" clearable @change="fetchDocuments">
          ...
          <el-select v-if="userStore.isHR || userStore.isAdmin" v-model="docFilters.status" placeholder="全部状态" clearable @change="fetchDocuments">
      ...
      <el-tab-pane label="标准答案库" name="faqs">
        <div style="display: flex; gap: 12px; margin-bottom: 16px">
          <el-input v-model="faqKeyword" placeholder="搜索问题..." clearable style="width: 300px" @keyup.enter="fetchFaqs">
            <template #append>
              <el-button :icon="Search" @click="fetchFaqs" />
            </template>
          </el-input>
          <el-select v-model="faqCategory" placeholder="全部分类" clearable @change="fetchFaqs">
            <el-option label="考勤" value="attendance" />
            <el-option label="薪酬" value="salary" />
            <el-option label="福利" value="benefit" />
            <el-option label="休假" value="leave" />
    ...
    <!-- 文档详情抽屉 -->
    <el-drawer v-model="docDetailVisible" title="文档详情" size="60%">
    ...
    <!-- 文档编辑对话框 -->
    <el-dialog v-model="docDialogVisible" :title="docEditId ? '编辑文档' : '上传文档'" width="700px">
    ...
    <!-- FAQ 编辑对话框 -->
    <el-dialog v-model="faqDialogVisible" :title="faqEditId ? '编辑标准答案' : '新增标准答案'" width="600px">
